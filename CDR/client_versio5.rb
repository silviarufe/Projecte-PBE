...
def query_table
  ...
  if result.is_a?(Array) && !result.empty?
    # Obtenir els titols de la taula utilitzant les claus del primer element
    header = result.first.keys.join(" | ")
    
    # Generar les linies de resultats
    result_lines = result.map do |item|
      item.values.join(" | ")
    end.join("\n")
    
    # Combinar els titols i els resultats
    result_text = "#{header}\n#{result_lines}"
    
    # Mostrar els resultats al quadre de text
